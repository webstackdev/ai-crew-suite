...
import type { ShadowDetectiveApi } from './apiRef';

/** Stable AI Core agent ID for report-only cloud reconciliation. */
export const SHADOW_DETECTIVE_AGENT_ID = 'scaffolder-ai-shadow-detective';

/** Authenticated HTTP/SSE client for live and replayed shadow reports. */
export class ShadowDetectiveClient implements ShadowDetectiveApi {
  ...
  private async fetchStream(
    ...
    if (!response.ok)
      throw new Error(`Unable to retrieve shadow report from ${path}`);
