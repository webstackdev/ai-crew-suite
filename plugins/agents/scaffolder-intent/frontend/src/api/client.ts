...
import type { AiRunEvent, IntentRequest, StartIntentInput } from '../@types';
import type { ScaffolderIntentApi } from './apiRef';

/** Stable AI Core agent identifier for schema-grounded template proposals. */
export const SCAFFOLDER_INTENT_AGENT_ID = 'scaffolder-ai-intent';

/** Authenticated HTTP/SSE client for live and replayed intent proposals. */
export class ScaffolderIntentClient implements ScaffolderIntentApi {
  ...
  private async fetchStream(
    ...
    if (!response.ok)
      throw new Error(
        `Unable to retrieve template intent proposal from ${path}`,
      );
