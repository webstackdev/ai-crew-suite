  ...
  register(env) {
    env.registerInit({
      ...
      async init({ logger, urlReader, config, vcsRegistry }) {
        logger.info('Initializing decoupled Gerrit VCS driver module...');

        const integrations = ScmIntegrations.fromConfig(config);

        const gerritDriver = new GerritDriver({
          urlReader,
          logger: logger.child({ label: 'vcs-gerrit' }),
          integrations,
        });

        vcsRegistry.registerDriver(gerritDriver);
      },
    });
  },
});

export default aiCoreBackendModuleVcsGerrit;
