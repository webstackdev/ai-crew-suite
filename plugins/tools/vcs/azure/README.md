...
This backend module registers an Azure DevOps `VcsDriver` with the host plugin **`@ai-crew-suite/plugin-tool-vcs-backend`** through its exposed `vcsDriversExtensionPoint`.
...
```typescript
import { toolVcsModuleAzure } from '@ai-crew-suite/plugin-tool-vcs-backend-module-azure';
...
yarn workspace @ai-crew-suite/plugin-tool-vcs-backend-module-azure build
```

### 3. Running Unit & Integration Tests

```bash
yarn workspace @ai-crew-suite/plugin-tool-vcs-backend-module-azure test
```
