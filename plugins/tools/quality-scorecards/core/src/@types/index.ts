...
export type ScorecardId = 'soundcheck' | 'sonarqube' | 'custom';

export type QualityScorecardsConfig = {
  /** The driver currently selected for active tools routing fallbacks. */
  provider: string;
};
...
export type TechRadarProposalInput = {
  quadrantId: string; // e.g., 'languages', 'frameworks'
  ringId: string;     // e.g., 'assess', 'trial', 'adopt', 'hold'
  title: string;
  description: string;
  reason: string;
};
...
/**
 * Provider-neutral driver interface for quality metric evaluation and architecture radars.
 */
export interface QualityScorecardsDriver {
  readonly providerId: string;

  /**
   * Retrieves structural health indicators and rule matrices for a target Backstage component entity.
   * Leveraged primarily by tech-debt-ai-scout.
   */
  getEntityScorecard(entityRef: string): Promise<EntityScorecardSummary>;

  /**
   * Submits a structural change request modifying adopted standard boundaries.
   * Leveraged primarily by tech-radar-ai-manager.
   */
