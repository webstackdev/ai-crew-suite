...
import type { AiRunEvent, ApprovalDecision, EvaluateRequestInput, GuardrailRequest } from '../@types';
...
export class ScaffolderGuardrailClient implements ScaffolderGuardrailApi {
  ...
  async *evaluateRequest(input: EvaluateRequestInput): AsyncGenerator<AiRunEvent> { const request: GuardrailRequest = { version: 1, source: 'manual', ...input }; yield* this.read(await this.stream(`agents/${SCAFFOLDER_GUARDRAIL_AGENT_ID}/runs`, { method: 'POST', headers: { 'content-type': 'application/json', ...(await this.headers()) }, body: JSON.stringify({ input: { query: JSON.stringify(request) } }) })); }
