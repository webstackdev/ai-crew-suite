...
import type { AiRunEvent, ImpactRequest, StartImpactInput } from '../@types';
import type { SearchContextApi } from './apiRef';

/** Stable AI Core agent identifier for source-change impact assessments. */
export const SEARCH_CONTEXT_AGENT_ID = 'search-ai-context';

/** Authenticated HTTP/SSE client for live and replayed impact assessments. */
export class SearchContextClient implements SearchContextApi {
  ...
  private async fetchStream(
    ...
    if (!response.ok)
      throw new Error(`Unable to retrieve impact assessment from ${path}`);
