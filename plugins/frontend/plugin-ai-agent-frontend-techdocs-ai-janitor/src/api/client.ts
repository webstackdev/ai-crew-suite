...
import type { AiRunEvent, JanitorRequest, StartJanitorInput } from '../@types';
import type { TechdocsJanitorApi } from './apiRef';

/** Stable AI Core agent identifier for TechDocs audits. */
export const TECHDOCS_JANITOR_AGENT_ID = 'techdocs-ai-janitor';

/** Authenticated HTTP/SSE transport for scoped TechDocs audits. */
export class TechdocsJanitorClient implements TechdocsJanitorApi {
  ...
  private async fetchStream(
    ...
    if (!response.ok)
      throw new Error(`Unable to retrieve TechDocs audit from ${path}`);
