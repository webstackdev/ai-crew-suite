...
/**
 * Tests whether a firing overlaps a real-signal window, padded on both sides.
 *
 ...
 * @param padMinutes - Symmetric tolerance applied to the window bounds.
 */
...
/**
 * Applies real-signal suppression to a computed noise score.
 *
 ...
 * This is the guard that prevents the agent from tuning away a real failure
 * signal, so it deliberately overrides even a strongly noisy fingerprint.
 *
...
export const applySuppression = (
  ...
  return { ...score, verdict: 'real_signal', suppressedBy: [...new Set(suppressedBy)] };
};

/** Shape shared by incident and deploy records returned from read-only tools. */
type CorrelationRecord = Record<string, unknown>;

const optionalString = (record: CorrelationRecord, ...keys: string[]) => {
