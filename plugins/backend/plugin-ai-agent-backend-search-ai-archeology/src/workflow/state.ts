...
export type ArcheologyRequest = { version: 1; source: 'manual'; question: string; entityRef?: string; repoUrl?: string; paths?: string[]; since?: string; until?: string; sessionId?: string };
...
export type ContributionEvidence = { id: string; kind: 'authored' | 'reviewed' | 'triaged' | 'commented'; actor: ServiceActor; at: string; reference?: string };
/** Explicit identity outcome retained even when an actor is no longer reachable. */
export type ResolvedIdentity = { actor: ServiceActor; status: 'active' | 'moved_team' | 'offboarded' | 'unresolved'; userRef?: string; displayName?: string; groupRefs: string[]; evidence: string[] };
