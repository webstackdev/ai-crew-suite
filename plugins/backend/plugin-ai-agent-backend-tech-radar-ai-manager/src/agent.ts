...
export const TECH_RADAR_SYSTEM_PROMPT = 'Use only parsed radar entries and direct manifest dependencies. Never infer rings, claim durable submission, or submit a radar proposal.';
/** Creates the stateless read-only technology radar agent. */
export const createTechRadarAgent = (config: TechRadarConfig): AgentDefinition => ({ id: TECH_RADAR_AGENT_ID, modelRef: config.modelRef, workflowRef: TECH_RADAR_WORKFLOW_ID, memory: 'none', systemPrompt: TECH_RADAR_SYSTEM_PROMPT, toolIds: [...TECH_RADAR_TOOL_IDS], triggers: [{ id: 'tech-radar-analysis-on-demand', source: 'manual', agentId: TECH_RADAR_AGENT_ID }] });
