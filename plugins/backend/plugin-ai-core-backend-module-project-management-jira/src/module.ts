  ...
  register(env) {
    env.registerInit({
      deps: {
        ...
        ticketRegistry: projectManagementDriversExtensionPoint,
      },
      async init({ config, logger, ticketRegistry }) {
        logger.info('Initializing decoupled Jira project management driver module...');

        ticketRegistry.registerDriver(
          new JiraDriver({
            logger: logger.child({ label: 'project-management-jira-driver' }),
