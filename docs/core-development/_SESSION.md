...
1. We have a 'usage' event that feeds to infrastructure logging to track platform spend, token consumption, and prompt efficiency across your engineering organization.

In Section 5.2 "Node harness — where the safety lives" we have this: "**Budget accounting**: wall-clock per node, cumulative token budget, cumulative tool-invocation count; exceeding any emits `budget_exceeded` and aborts the run via LangGraph cancellation, not a thrown string."

Where is it located? Is it in a plugin-backend-core postgresql table?

Are we making use of this currently? Would it make sense to provide a way to monitor this maybe in a dedicated plugin?

2. In the same section, we have this: "**Redaction**: the existing `SENSITIVE_KEYS` redactor moves from `AgentRuntime` into the harness and applies to state patches, tool args/results in events, and checkpoint payloads. Secrets must never enter graph state: channels hold *references* (artifact refs, evidence IDs), and the harness rejects checkpoint payloads containing credential-shaped strings (regex sweep as defense-in-depth, tested)."

Do we have config where users can add / remove regex's for sensitive keys / secrets checking?
