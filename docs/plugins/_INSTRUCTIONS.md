...
Next let's generate docs/plugins/rfc-adr-ai-reviewer.md from the plugins/backend/plugin-ai-agent-backend-rfc-adr-ai-reviewer/_IMPLEMENTATION.md. Let's continue making sure the code base stays the source of truth if the completed implementation plan varies from it, and we add a Roadmap section at the end if we can identify work that was out of scope for the initial implementation.
